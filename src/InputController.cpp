...
InputController::InputController()
    : m_camera(nullptr),                   // 绑定要控制的相机
      m_moveSpeed(DEFAULT_MOVE_SPEED),     // 默认移动速度
      m_rotateSpeed(DEFAULT_ROTATE_SPEED), // 默认鼠标旋转灵敏度
      m_zoomSpeed(DEFAULT_ZOOM_SPEED),     // 默认滚轮缩放速度
      m_invertY(DEFAULT_INVERT_Y),         // 默认不反转鼠标 Y 轴
      m_firstMouse(true),                  // 初始鼠标状态
      m_lastX(0.0),                        // 鼠标上一次 X 坐标
      m_lastY(0.0),                        // 鼠标上一次 Y 坐标
      m_leftButtonPressed(false),          // 默认左键未按下
...
void InputController::onMouseMove(double xpos, double ypos)
{
    ...
    if (m_camera)
    {
        m_camera->rotate(static_cast<float>(-yoffset * m_rotateSpeed), static_cast<float>(-xoffset * m_rotateSpeed), 0.0f);
    }
}
